...
export default class extends Component {
  ...
  renderPagination = () => {
    ...
    const dots = [];
    const ActiveDot = this.props.activeDot || <View
      style={[{
    ...
    />;
    const ActivePlusDot = <Text style={{ color: '#007aff', fontWeight: 'bold' }}>+</Text>;
    const PlusDot = <Text style={{ color: 'rgba(0,0,0,.2)', fontWeight: 'bold' }}>+</Text>;
    const childs = this.props.children;

    for (let i = 0; i < this.state.total; i++) {
      if (childs[i].props.plusDot.indexOf('MORE') > -1 || childs[i].props.plusDot.indexOf('¡MÁS!') > -1) {
        dots.push(i === this.state.index
          ? React.cloneElement(ActivePlusDot, { key: i })
          : React.cloneElement(PlusDot, { key: i }),
        );
      } else {
        dots.push(i === this.state.index
